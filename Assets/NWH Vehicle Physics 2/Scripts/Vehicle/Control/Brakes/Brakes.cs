...
namespace NWH.VehiclePhysics2
{
    ...
    public class Brakes : VehicleComponent
    {
        ...
        public override void FixedUpdate()
        {
            ...
            // **** Everything after this point will set the brake flag and thus register as braking. ****
            _isBraking = false;
            
            int currentGear = vc.powertrain.transmission.Gear;
            if (brakeOffThrottle && throttleInput < 0.05f)
            {
            ...
            // Handbrake
            if (vc.input.Handbrake > Input.Input.DEADZONE && vc.IsAwake)
            {
                foreach (WheelGroup axle in vc.WheelGroups)
                {
                    if (axle.handbrakeCoefficient > 0)
            ...
            bool idleBrake = brakeWhileIdle && throttleInput < Input.Input.DEADZONE && currentGear == 0 && vc.Speed < 0.3f;
            ...
            // Brake on input
            if (brakeInput > Input.Input.DEADZONE)
            {
                _intensity = Mathf.SmoothDamp(_intensity, brakeInput, ref _intensityVelocity, smoothing);
